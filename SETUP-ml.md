...
## Быстрый вариант: только `scikit-learn`, без venv

Если пока не готовы ставить весь ML-стек (venv 3.12 + pyannote), но хотите
починить смешивание мужского/женского голоса — достаточно ОДНОГО пакета,
прямо в текущем окружении на Python 3.14:

```bash
pip3 install scikit-learn
```

`torch`/`torchaudio` у вас уже есть. После установки `app.py` при резервной
диаризации (когда pyannote не задействован) сам скачает модель WavLM
(`torchaudio.pipelines.WAVLM_BASE`, публичные веса, автоматически при первом
запуске) и переразметит, кто говорит в каждой уже найденной Gemini реплике —
**не трогая тайминг**: границы реплик остаются ровно те, что определил Gemini,
меняется только ярлык говорящего. В логе появится:

```
[dubbing] диаризация переразмечена по WavLM-эмбеддингам (gate=..., turn'ов: N)
```

Если уверенность разметки низкая, в лог пишется:

```
[dubbing] переразметка по эмбеддингам неуверенная (gate=...) — оставляю разметку Gemini
```

— в этом случае ничего не меняется, ошибку это не даёт.

## Что включится автоматически (полный ML-стек)
